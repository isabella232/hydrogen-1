...
export type CookieOptions = {
   ...
   * Some browsers [don't work with secure cookies on localhost]
  ...
  */
  secure?: boolean;
  /** Declares that the cookie should be restricted to a first-party or
  ...
  */
  path?: string;
  /** [A date in which the cookie will expire]
  ...
  */
  expires?: Date;
  /** Secures the cookie so that it's only used on [specific domains]
...
/** The `Cookie` component helps you build your own custom cookie and session implementations.
 * All [Hydrogen session storage mechanisms](/custom-storefronts/hydrogen/framework/sessions#types-of-session-storage)
 * use the same configuration options as what's available in `Cookie`.
 */
export class Cookie {
  name: string;
